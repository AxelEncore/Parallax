...
public record TextAnnotationData(
   String text, Vector3f position, Quaternionf rotation, Direction direction, float fallbackYaw, float scale, int billboardMode, int colour, boolean shadow
   ...
   public TextAnnotationData withText(String text) {
   ...
   public static TextAnnotationData read(FriendlyByteBuf friendlyByteBuf) {
      ...
      return new TextAnnotationData(
         text, new Vector3f(x, y, z), new Quaternionf(rotX, rotY, rotZ, rotW), direction, fallbackYaw, scale, billboardMode, colour, shadow
      );
   }
}
