...
public class PyramidRasterization {
         ...
         for (int x = -maxRadiusX; x <= maxRadiusX; x++) {
            for (int y = -maxRadiusY; y <= maxRadiusY; y++) {
               for (int z = -maxRadiusZ; z <= maxRadiusZ; z++) {
                  ...
                  float r = Math.max(Math.abs(rx) * radiusZ, Math.abs(rz) * radiusX) * height;
                  if (r <= h2 * radiusX * radiusZ && Math.abs(ry) <= radiusY + 0.5F) {
                     if (!hollow) {
                        ...
                        for (int i = 0; i < 18; i += 3) {
                           ...
                           h2 = radiusY - ry + 1.0F;
                           r = Math.max(Math.abs(rx) * radiusZ, Math.abs(rz) * radiusX) * height;
                           if (r > h2 * radiusX * radiusZ || Math.abs(ry) > radiusY + 0.5F) {
                              region.addBlock(x + centerX, y + centerY, z + centerZ, block);
                              break;
                           }
   ...
   public static void pyramidMetaball(
      ChunkedBlockRegion region,
      int metaballRange,
      BlockState block,
      BlockPos center,
      ...
      if (metaballRange > 1 && level != null) {
         ...
         pyramid(region, block, center, diameterX, height, diameterZ, hollow, centerEvenDiameters, quaternionf);
      }
   }
}
