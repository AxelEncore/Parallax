...
public class CylinderRasterization {
         ...
         for (int x = -maxRadiusX; x <= maxRadiusX; x++) {
            for (int y = -maxRadiusY; y <= maxRadiusY; y++) {
               for (int z = -maxRadiusZ; z <= maxRadiusZ; z++) {
                  ...
                  if (rx * rx * invRadiusSqX + rz * rz * invRadiusSqZ <= 1.0F && Math.abs(ry) <= radiusY + 0.5F) {
                     if (!hollow) {
   ...
   public static void cylinderMetaball(
      ChunkedBlockRegion region,
      int metaballRange,
      BlockState block,
      BlockPos center,
      ...
      if (metaballRange > 1 && level != null) {
         ...
         cylinder(region, block, center, diameterX, height, diameterZ, hollow, centerEvenDiameters, quaternionf);
      }
   }
}
