...
public class CloneBuilderTool implements BuilderTool {
      ...
      if (this.blockRegion != null) {
         if (this.reclone) {
            BuilderTool.renderKeybindHelp(
               guiGraphics, screenWidth, screenHeight, AxiomI18n.get("axiom.buildertool.clone.finish"), Minecraft.getInstance().options.keyAttack, 0
            );
   ...
   public void renderWorld(AxiomWorldRenderContext rc) {
      if (this.regionOffset != null) {
         ...
         ChunkedBlockRegion newBlockRegion = BuilderTool.handleMoveableBlockBuffer(
            rc, this.blockRegion, this.regionOffset, this.transformMatrix, rotatePressed, flipPressed
         );
   ...
   private void apply() {
      if (this.regionOffset != null) {
         ...
         String description = AxiomI18n.get("axiom.history_description.cloned", NumberFormat.getInstance().format((long)this.blockRegion.count()));
         ...
         if (!this.blockEntities.isEmpty() && !this.transformMatrix.isIdentity()) {
            ...
            ObjectIterator additionalUndoOperation = this.blockEntities.long2ObjectEntrySet().iterator();

            while (additionalUndoOperation.hasNext()) {
               Entry<CompressedBlockEntity> entry = (Entry<CompressedBlockEntity>)additionalUndoOperation.next();
               ...
               if (!this.blockRegion.getBlockStateOrAir(nx, ny, nz).isAir()) {
         ...
         if (level != null && ClientEvents.serverSupportsProtocol(SupportedProtocol.CREATE_ENTITY) && BuilderToolManager.copyEntities) {
            ...
            if (!entities.isEmpty()) {
               ...
               for (Entity entity : entities) {
                  ...
                  spawnEntries.add(new AxiomServerboundSpawnEntity.SpawnEntry(newUuid, position, newYaw, newPitch, entity.getUUID(), null));
                  uuids.add(newUuid);
               }
         ...
         RegionHelper.pushBlockRegionChangeOffset(this.blockRegion, blockEntities, offset, description, flags, additionalUndoOperation);
