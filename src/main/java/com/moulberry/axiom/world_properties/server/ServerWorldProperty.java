...
public class ServerWorldProperty<T> {
   ...
   public ServerWorldProperty(ResourceLocation id, String name, boolean localizeName, WorldPropertyWidgetType<T> widget, T value, Predicate<T> handler) {
      this.id = id;
      this.name = name;
   ...
   public void update(ServerLevel serverLevel, byte[] data) {
      ...
      if (this.handler.test(this.value)) {
         AxiomClientboundSetWorldProperty packet = new AxiomClientboundSetWorldProperty(
            this.id, this.widget.dataType().getTypeId(), this.widget.dataType().serialize(this.value)
         );
