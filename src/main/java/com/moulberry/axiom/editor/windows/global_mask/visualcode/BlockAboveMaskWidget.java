...
public class BlockAboveMaskWidget extends MaskWidget {
   private final BlockConditionWidget blockConditionWidget = new BlockConditionWidget(Blocks.STONE);

   public BlockAboveMaskWidget() {
   }

   public BlockAboveMaskWidget(BlockConditionWidget.BlockConditionState state) {
      this.blockConditionWidget.revertState(state);
   }

   @Override
   public MaskWidget copy() {
      return new BlockAboveMaskWidget(this.blockConditionWidget.createState());
   }

   @Override
   public void doRender(boolean allowDragDropSource, BooleanWrapper allowDragDropTarget) {
      ImGui.button(AxiomI18n.get("axiom.editorui.window.tool_masks.mask_above") + " =", 0.0F, (int)(52.0F * EditorUI.getUiScale()));
