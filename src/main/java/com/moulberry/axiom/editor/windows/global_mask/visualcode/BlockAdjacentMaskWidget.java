...
public class BlockAdjacentMaskWidget extends MaskWidget {
   private final BlockConditionWidget blockConditionWidget = new BlockConditionWidget(Blocks.STONE);

   public BlockAdjacentMaskWidget() {
   }

   public BlockAdjacentMaskWidget(BlockConditionWidget.BlockConditionState state) {
      this.blockConditionWidget.revertState(state);
   }

   @Override
   public MaskWidget copy() {
      return new BlockAdjacentMaskWidget(this.blockConditionWidget.createState());
   }

   @Override
   public void doRender(boolean allowDragDropSource, BooleanWrapper allowDragDropTarget) {
      ImGui.button(AxiomI18n.get("axiom.editorui.window.tool_masks.mask_adjacent") + " =", 0.0F, (int)(52.0F * EditorUI.getUiScale()));
